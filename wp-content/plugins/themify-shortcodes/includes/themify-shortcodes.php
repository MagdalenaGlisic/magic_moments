	...
	switch ( $code ) {
		...
		case 'themify_button':
			...
			if($atts['color']!=='' || $atts['text']!=='' ){
			    $html.=' style="';
			    if($atts['color']!=='' ){
				$html.= "background-color: {$atts['color']};";
			    }
			    if( $atts['text']!=='' ) {
				$html.= "color: {$atts['text']};";	
			    }
			    $html.='"';
			}
			if( $atts['target']!=='' ) {
		...
		case 'themify_hr':
			...
			if( '' !== $atts['width'] || '' !==$atts['border_width']){
				$hrstyle = 'style="';
				if( '' !== $atts['width']  ){
					$hrstyle .= 'width:' . $atts['width'] . ';';
				}
				if( '' !== $atts['border_width']  ){
					if( strpos('/MSIE 7/i', $_SERVER['HTTP_USER_AGENT'] )!==false ){
						$hrstyle .= 'height:' . $atts['border_width'] . ';';
					}
					$hrstyle .= 'border-width:' . $atts['border_width'] . ';';
				}
				$hrstyle .= '"';
			} else {
				$hrstyle = '';
		...
		case 'themify_map':
			...
			<div class="themify-shortcodes-map-wrap">
				<div
					...
					data-zoom="' .$atts['zoom']  . '"
					data-type="' . $atts['type'] . '"
					data-scroll="' . $atts['scroll'] . '"
					data-drag="' . $atts['draggable']  . '"
					id="' . uniqid( 'themify_map_canvas_' ) . '"
					style="display: block; width:' .  $atts['width']  . '; height:' . $atts['height']  . ';" class="themify-shortcodes-map"
				></div>
			</div>';
		break;
		case 'themify_video':
			...
			if ( isset( $atts['src'] ) ) {
				return do_shortcode( '[video src="' . $atts['src'] . '"]' );
			}
		break;
	}
	return '';
...
function themify_shortcode_list_posts( $atts, $content = null ) {
	...
	if ( $posts ) {
		...
		$out.='<div class="loops-wrapper shortcode clearfix list-posts layout ' . esc_attr( $themify->post_layout ) . ' tf_clear">';
...
function themify_shortcode_slider( $atts, $content = null ){
	...
	$strsldr = '<!-- shortcode slider --><div id="slider-' . esc_attr( $js_data['numsldr'] ) . '" class="themify-shortcodes-slider shortcode clearfix slider ' . esc_attr( $class ) . '">
...
function themify_shortcode_post_slider( $atts, $content = null ) {
	...
	if ($posts) {
		...
		$js_data['height'] = $height;
		$js_data['numsldr'] = rand(0,1011).uniqid();
		$postsliderstr = '<!-- shortcode post_slider --> <div id="slider-' . esc_attr(  $js_data['numsldr'] ) . '" style="width: ' . esc_attr( $width ) . ';" class="themify-shortcodes-slider shortcode clearfix post-slider ' . $class . '">
...
function themify_shortcode_author_box( $atts, $content = null ) {
	...
	$authorboxstr = '<!-- shortcode author_box --> <div class="shortcode clearfix author-box ' . esc_attr( $atts['style'] . ' ' . $nicename ) . ' ">';
	if ( 'yes' === $atts['avatar'] ) {
...
function themify_shortcode_box( $atts, $content = null ) {
	...
	return '
	<!-- shortcode box --> <div class="shortcode clearfix box ' . esc_attr($atts['style'] ) . '">'
...
function themify_shortcode_icon( $atts, $content = null ) {
	...
	if ( ! empty( $atts['icon'] ) ) {
		if ( class_exists( 'Themify_Icon_Font' ) ) {
			...
			$out .= '<i class="themify-icon-icon fa ' . $atts['icon'] . '" ' . $style_attr . '></i>';
		}
	}
