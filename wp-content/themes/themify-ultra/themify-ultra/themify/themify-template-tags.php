...
if (!function_exists('themify_logo_image')) :
	...
	if ($logo_is_image === true && empty($logo_mod)) {
	    ...
	    $html .= '<a href="' . esc_url(apply_filters('themify_logo_home_url', home_url())) . '" title="' . esc_attr($site_name) . '">';
	    $type = $logo_is_image === true ? 'image' : 'text';
	    if ('image' === $type) {
		$html .= themify_get_image(array('src' => themify_get('setting-' . $location . '_image_value', '', true), 'w'=>themify_get('setting-' . $location . '_width', '', true), 'h'=>themify_get('setting-' . $location . '_height', '', true) , 'alt'=> $site_name,'disable_lazy'=>Themify_Enqueue_Assets::$isFooter === false));
...
if (!function_exists('themify_zoom_icon')) :
	...
	$zoom = apply_filters('themify_zoom_icon', themify_check('lightbox_icon') ? '<span class="zoom">'.themify_get_icon('search','ti').'</span>' : '');
	if ($echo !== true) {
	    return $zoom;
	}
	echo $zoom;
...
if (!function_exists('themify_get_pagenav')){
	...
	if ($max_page > 1) {
		...
		if (empty($paged)) {
		    $paged=(int)get_query_var( 'page',1 );
		}
	    }
	    ...
	    $out = $before . '<div class="pagenav tf_clear tf_box tf_textr clearfix">';
	    if($start_page <= 0){
		$start_page=1;
	    }
	    ...
	    $out .= '</div>' . $after;
        $prefetch=class_exists('Themify_Builder') && Themify_Builder::$frontedit_active===true?'':$prefetch;
	    return $prefetch . $out;
	}
	return '';
    }
...
if (!function_exists('themify_area_design')){
	...
	if(!isset($args['field']) && !is_singular()){
...
if (!function_exists('themify_theme_feed')){
	...
	if(!themify_check($key,true)){
	    ...
	    ?>
	    <div class="rss<?php if(isset($args['class'])):?> <?php echo $args['class']?><?php endif;?>"><a<?php if(isset($args['link_class'])):?> class="<?php echo $args['link_class']?>"<?php endif;?> href="<?php themify_theme_feed_link($key) ?>"><?php if(isset($args['icon'])):?><?php echo themify_get_icon($args['icon'])?><?php endif;?><?php echo isset($args['text'])?$args['text']:__('RSS','themify')?></a></div>
	    <?php
	}
    }
}
...
if (!function_exists('themify_post_media')){
    ...
    function themify_post_media($args = array()) {
		...
		if ($themify->hide_image !== 'yes') {
			$isImage = 'yes' !== $themify->unlink_image || isset( $_GET['post_in_lightbox']);
			if(isset($args['image'])){
			    ...
			    if ($post_image==='') {
				    $post_image = themify_get_image(array('w'=>$themify->width,'h'=>$themify->height,'class'=>(isset($args['image_class'])?$args['image_class']:'')));
			...
			?>
			<figure class="<?php echo isset($args['class']) ? $args['class'] : 'post-image clearfix'?><?php if($isImage===false):?> is_video<?php endif;?>">
			    <?php 
	...
	elseif ( false !== stripos( $video_url, 'vimeo' ) ) {
		...
		if(!$data){
			...
			set_transient( $key,array('t'=>$title,'u'=>$image_url),MONTH_IN_SECONDS);
		    }
		}
		else{
		    $image_url = $data['u'];
...
if (!function_exists('themify_post_content')){
	...
	<div class="entry-content">
		...
		<?php if (themify_check('setting-excerpt_more', true) && !is_single()) : ?>
	    
		    <p><a href="<?php $link=themify_permalink_attr(array(),false); echo $link['href']; ?>" class="more-link"><?php echo themify_get('setting-default_more_text', __('More &rarr;', 'themify'), true) ?></a></p>
		...
		?>
		<?php  the_content(themify_get('setting-default_more_text', __('More &rarr;', 'themify'), true)); ?>
...
if (!function_exists('themify_post_title_tag')){
	...
	global $themify;
	$tag = (empty($themify->is_shortcode) && empty($themify->is_builder_loop) && is_singular()) ? 'h1' : 'h2';
...
if (!function_exists('themify_comments_popup_link')){
    ...
    function themify_comments_popup_link(array $args=array()){
		...
		if($themify->hide_meta!=='yes' && ( !isset( $themify->hide_meta_comment ) || $themify->hide_meta_comment!=='yes' ) && comments_open()){
			...
			if(( $post_type!=='portfolio' && themify_check( 'setting-comments_posts',true ) ) || ( $post_type==='portfolio' && !themify_check( 'setting-portfolio_comments',true ) )){
				return;
			}
		}
	    else{
...
if (!function_exists('themify_author_bio')) {
	...
	<div class="author-bio clearfix">
	    <p class="author-avatar"><?php echo get_avatar($curauth->user_email, 48); ?></p>
...
if(!function_exists('themify_masonry_filter')){
    function themify_masonry_filter(){
	global $themify;
	if ('slider' !== $themify->post_layout && !empty($themify->post_filter) && $themify->post_filter !== 'no') {
	    $post_type= $themify->query_category !== ''?$themify->query_post_type:get_post_type();
	    get_template_part('includes/filter',$post_type);
	    return true;
	}
	return false;
    }
...
function themify_set_loop_args(array $class,$post_type,$layout,$type='main'){
    ...
    $container_props = apply_filters('themify_container_props', array(
		'id' => 'loops-wrapper',
		'class' => $class,
		'data-lazy'=>1
    ), $post_type, $layout,$type);
    ...
    $container_props['class'][]='tf_clear clearfix';
    ...
    function themify_page_content($args=array()){
		...
		elseif(themify_is_page()){
			if (have_posts()) {
				...
				?>
				<div id="page-<?php the_ID(); ?>" class="type-page" itemprop="articleBody">
					<?php 
					if(!isset($args['hide_title']) || $args['hide_title']===false){
...
if (!function_exists('themify_comment_list')) {
	...
	<li id="comment-<?php comment_ID() ?>" <?php comment_class(); ?>>
		...
		<cite><?php echo themify_get_icon('bookmark','fa'); ?><?php echo get_comment_author_link(); ?></cite>
		<br/>
...
function themify_menu_nav( $args = array(),$reinit=false ) {
    $args=themify_parse_args( $args, array(
		...
		'menu'=>'',
		'menu_id'     => 'main-nav',
		'menu_class'  => 'main-nav clearfix tf_box'
    ));
    $cacheDisable=$reinit===true  || TFCache::get_cache_plugins('any') || themify_check('setting-cache-menu',true) || themify_check('setting-cache-html',true);    
    ...
    if($cacheDisable===false){
	    ...
	    $transient_key.='_'.md5(json_encode($args));
    ...
    if(empty($transient)){
		...
		if(empty($args['walker'])){
			add_filter('nav_menu_item_args', 'themify_menu_child_arrow',100,3);
		}
		...
		if($cacheDisable===false){
			set_transient($transient_key, !$menu?'empty':$menu,MONTH_IN_SECONDS);
		}
		remove_filter( 'nav_menu_css_class', 'themify_class_to_nav_menu', 100, 2 );
		remove_filter('nav_menu_item_args', 'themify_menu_child_arrow',100,3);
    }
    else{
		$menu=$transient;
    }
...
function themify_menu_child_arrow( $args, $item, $depth){
	$args->link_after=in_array('menu-item-has-children', $item->classes,true)?'<span class="child-arrow closed"></span>':'';
	return $args;
}
if ( ! function_exists( 'themify_default_main_nav' ) ) {
	...
	function themify_default_main_nav() {
		echo '<ul id="main-nav" class="main-nav clearfix tf_box">';
