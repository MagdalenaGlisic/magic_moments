...
class TFCache {
	...
	if (!isset($queue['blog']) &&Themify_Filesystem::is_dir($cache_dir)) {
		...
		if (!isset($queue[$k])) {
		    ...
		    if($type==='comment' || $type==='term' || $type==='category'){
				...
				$find[]=$type === 'page' ? ' page-id-' . $item_id : ' postid-' . $item_id; //if there is any html associated with updated post  
				$find[]=' post-' . $item_id;
		    }
		    set_time_limit(0);
			$type=$item_id=null;
    ...
    private static function clear_recursive($cache_dir,array $find){
		...
		while($f = readdir($dirHandle)){
			...
			elseif(strpos($item, '.html', 5) !== false && strpos($item, '.html.gz', 5)===false && is_file($item)){
			    $content = file_get_contents($item, FALSE, NULL, 1000);
			    if (!empty($content)){
					foreach($find as $v){
    ...
    public static function hooks() {
		...
		if(isset($_GET['tf-cache']) && $_GET['tf-cache']==='2' && is_user_logged_in()){
			add_action('wp_head', array(__CLASS__, 'clear_css_cache'), 1);
		}
    }
    ...
    public static function run(){
		...
		if(is_dir(self::$cache_dir) || mkdir(self::$cache_dir)){
			...
			if($isMulti===false || is_dir(self::$cache_dir) || mkdir(self::$cache_dir)){
				$request=$_SERVER['REQUEST_URI'];
				if(defined('TF_CACHE_RULES') && !empty(TF_CACHE_RULES)){
				...
				if(is_file(self::$cache_dir)){
					...
					if($liveTime>time()){
						...
						if(is_file(self::$cache_dir)){//maybe another proccess has already removed it?
							...
							header('Content-Length: '.filesize(self::$cache_dir));
				...
				add_filter( 'template_include', array( __CLASS__, 'template_include' ), 0 );
			}
			else{
				self::$cache_dir=null;
			}
	...
	public static function is_safari(){
		return strpos($_SERVER['HTTP_USER_AGENT'], 'Chrome')===false && strpos($_SERVER['HTTP_USER_AGENT'], 'Safari')!==false;
	}
    ...
    private static function tf_cache_start() {
		if(self::$stopCache===true || is_user_logged_in() || is_admin() || self::$cache_dir===null  || is_404() || is_search() || themify_is_ajax() || post_password_required() || is_trackback() || is_robots() || is_preview() || is_customize_preview() || themify_is_login_page()|| (themify_is_woocommerce_active() && (is_checkout() || is_cart() || is_account_page()))){
			return;
		}
		if(defined('TF_CACHE_IGNORE') && !empty(TF_CACHE_IGNORE)){
			$ignore=explode(',',trim(TF_CACHE_IGNORE));
			if(!empty($ignore)){
				foreach($ignore as $f){
				   if(is_callable($f) && (($f==='is_shop' && themify_is_shop()) || call_user_func($f))){
						return;
				   }
				}
			}
		...
		if(self::get_cache_plugins()){
			self::disable_cache();
			return;
		}
		define('TF_CACHE',true);
		ob_start();
		add_action('wp_footer',array(__CLASS__,'tf_body_end'),99999);
    }
    ...
    public static function tf_cache_end(){
		...
		if(self::$stopCache===false){
			...
			if(Themify_Filesystem::is_dir($dir) || mkdir($dir)){
				if(file_put_contents(self::$cache_dir,$html,LOCK_EX) && themify_check('setting-cache_gzip',true)){
	...
	if(Themify_Filesystem::is_writable($cache_dir)){
	    if(!empty($data['setting-cache-html']) && !self::get_cache_plugins()){ 
    ...
    public static function disable_cache(){
		...
		if(!is_multisite()){
			...
			if(Themify_Filesystem::is_file($config_f)){
				...
				if(!empty($content) && strpos($content,'class-themify-cache.php',10)!==false){
					$remove=true;
				}
			}
			else{
    ...
    public static function clearCacheOnUpdate($type,$name){
	    $isPlugin=$type==='plugin';
	    if(!is_multisite()){
		    Themify_Enqueue_Assets::clearConcateCss();
	    }
	    else{
		if($isPlugin===true && !function_exists('is_plugin_active')){
		    require_once( ABSPATH . 'wp-admin'.self::sep.'includes'.self::sep.'plugin.php' ); 
		}
		$sites=get_sites(array('fields'=>'ids','no_found_rows'=>true,'update_site_cache'=>false));
		foreach($sites as $s){
			switch_to_blog( $s );
			if(($isPlugin===true && is_plugin_active($name)) || ($isPlugin===false && get_template()===$name)){
				Themify_Enqueue_Assets::clearConcateCss('blog',false,$s);
			}
	...
	public static function cache_menu($wp_admin_bar){
		...
		if($hasCache===false && $cache_plugins==false){
			$hasMenuCache=true;
			$args[]=array(
				'id'=>'tf_clear_menu',
				'parent'=>'tf_clear_cache',
		...
		if(!empty($_GET['tf-cache'])){
			...
			elseif($cache_type===1){
				add_filter('themify_concate_css',array(__CLASS__,'clear_css'),10,2);
				if($hasCache===true){
	...
	public static function clear_css($f,$type){
		if(Themify_Filesystem::is_file($f)){
			unlink($f);
			clearstatcache();
		}
		return $f;
