...
f;a.body[0].removeAttribute("style");w.style.display="block";b=d.tb_app.Instances.Builder[0].$el.offset().top;b>e&&(e=b);d.scrollTo(0,e);a.iframe=n;a.is_builder_active=!0;setTimeout(function(){p.hide();for(var b=g.length-1;-1<b;--b)if(g[b]&&g[b].parentNode){var e=g[b].href,f=g[b].id;e?q[e]||-1!==e.indexOf("wp-includes")||-1!==e.indexOf("admin-bar")||(g[b].setAttribute("disabled",!0),g[b].parentNode.removeChild(g[b])):"tf_fonts_style"!==f&&"tf_lazy_common"!==f&&g[b].parentNode.removeChild(g[b])}g=
q=tbLoaderVars=u=null;k(".themify_builder_content,#wpadminbar,header").remove();p.filter("ul,a,video,audio").filter(":not(:has(link))").remove();b="scroll tfsmartresize debouncedresize throttledresize resize mouseenter keydown keyup mousedown assignVideo".split(" ");e=k(h);f=k(c);for(let c=b.length-1;-1<c;--c)e.off(b[c]),f.off(b[c]),a.body.off(b[c]);c.documentElement.removeAttribute("style");c.documentElement.removeAttribute("class");const m=d.tbLocalScript.ticks;m.postID=r;l.hasClass("tb_restriction")?
...
for(let a=b.length-1;-1<a;--a)h[b[a]]&&(h[b[a]]=null);a.events={};h.wp.emoji=null;a.cssLazy=[];for(let b=d.document.querySelectorAll("[data-lazy]"),a=b.length-1;-1<a;--a)b[a].removeAttribute("data-lazy")},3E3)},800)});const f=function(){d.themifyBuilder.post_ID=r;l=d.jQuery("body");l.trigger("builderiframeloaded.themify",b)};if("__rocketLoaderLoadProgressSimulator"in d){const a=setInterval(function(){"complete"===d.__rocketLoaderLoadProgressSimulator.simulatedReadyState&&(clearInterval(a),f())},10)
}else f()},{once:!0,passive:!0});c.body.appendChild(w)},!0);B()}}});a.body[0].classList.contains("tb_restriction")||-1!==h.location.href.indexOf("tb-id")||("#builder_active"===h.location.hash?(k(".toggle_tb_builder > a").first().click(),h.location.hash=""):(b=c.createElement("link"),b.href=m+(d?"&tb-id="+d:""),b.rel="prefetch",b.setAttribute("as","document"),c.head.appendChild(b)))};if(!0===h.loaded)v();else a.on("tf_init",v,!0)})(Themify,window,document);
