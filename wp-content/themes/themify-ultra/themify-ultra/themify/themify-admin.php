...
function themify_manage_links( $data = array() ) {
	...
	if ( $field_ids) {
		$field_ids=json_decode( $field_ids );
	} else {
		$field_ids = $start;
...
function themify_fontello_input_callback( $data = array() ) {
	...
	<div class="themify_field_row">
		<span class="label">'. __('Icon Font Package', 'themify') .themify_help(__( 'Go to <a href="http://fontello.com">fontello.com</a>, pick your icons, download the webfont zip, upload and insert the zip URL here. The icon package will be auto detected on Themify\'s icon library where you click "Insert Icon".', 'themify' )) . '</span>
...
if(!function_exists('themify_performance_settings')) {
	...
	function themify_performance_settings($data = array()){
		...
		$cache_plugins=TFCache::get_cache_plugins();
		...
		if(!class_exists('Imagick') && !function_exists('imagewebp')){
			...
			$message=__('Only local images will be converted. The CDN and external images can not be converted. For background images of Themify Builder, you have to regenerate CSS and .htaccess file must be writeable.','themify');
			$webp=themify_check( 'setting-webp',true );
		}
		$output.='<hr><p>
			<span class="label">' . __( 'WebP Image', 'themify' ) .themify_help(__('Using WebP image format can reduce file size by 50-90%. '. $message ,'themify')) . '</span>
		...
		$key='setting-css';
		$output.= '<hr><span class="label">' . __( 'Themify CSS Output', 'themify' ) .themify_help(__('Output CSS files in the footer will load page without render blocking, but it can cause render delay.','themify')) . '</span>';
		$value = themify_get( $key,'top',true );
		...
		$output .= __('In the footer', 'themify') . '</label></div>';
		return $output;
	}
}
...
function themify_google_map_key($data=array()){
    return '<p><span class="label">' . __( 'Google Map Key', 'themify' ) .themify_help(__('Google API key is required to use Builder Map module and Map shortcode.','themify').' <a href="//developers.google.com/maps/documentation/javascript/get-api-key#key" target="_blank">' . __( 'Generate an API key', 'themify' ) . '</a> '. __('and insert it here','themify')). '</span> <input type="text" class="width10" name="setting-google_map_key" value="' . esc_attr( themify_get( 'setting-google_map_key','',true) ) . '" /> <br /></p>';
}
