...
function themify_get( $var, $default = null, $data_only = false ) {
	...
	elseif ( $data_only!==true ) {
		global $post;
		if(is_object( $post ) ) {
		    $id=themify_is_shop() && !in_the_loop()?themify_shop_pageId():$post->ID;
...
function themify_is_minify_enabled(){
    ...
    if($is===null){
		...
		if($is===false ) {
		   $is = function_exists( 'themify_builder_get' ) && themify_builder_get( 'setting-script_minification-min','builder_minified' ) === null; 
...
function themify_is_login_page(){
	...
	if($is===false && isset( $_SERVER['SCRIPT_NAME'])){
	    $f=rtrim(TFCache::get_current_url(),'/');
	    $is=rtrim(wp_login_url(), '/')===$f || rtrim(wp_registration_url(), '/')===$f || rtrim(wp_lostpassword_url(), '/')===$f;
	}
    }
    return $is;
}
...
function themify_defer_js($tag,$handle,$src){
	if ( ! empty( $tag ) && strpos( $tag, ' defer' ) === false ) {
		static $isJquery=null;
		if($isJquery===null){
			$isJquery=themify_check('setting-jquery',true) && (!is_admin() || themify_is_ajax());
		}
		if($isJquery===true || $handle === 'admin-bar' || Themify_Enqueue_Assets::is_themify_file( $src, $handle ) || in_array( $handle, Themify_Enqueue_Assets::getKnownJs(), true )){
			static $ex=null;
			if($ex===null){
...
function themify_make_lazy($html,$load=true){ 
    if(isset($html) && (strpos($html,'src=')!==false || strpos($html,'<audio')!==false)){
		...
		if($isSafari===null){
			...
			$isSafari=$hasLazy===true && TFCache::get_cache_plugins()?!themify_check('setting-disable-lazy-native',true):TFCache::is_safari();	
		}
			...
			if(!empty($matches[0])){	
				...
				foreach($matches as $item){	
				    if(strpos($item,'data-lazy',4)===false && strpos($item,'data-tf-not-load',4)===false &&  strpos($item,'data-tf-src',4)===false && strpos($item,'data-src',4)===false){
					    ...
					    if($ext==='audio' || !empty($image_src[1])){
							$url=$ext==='audio'?true:trim($image_src[1]);
							unset($image_src);
							if($url!=='' && ($ext!=='img' || strpos($url,'data:image')===false)){
								...
								if($ext==='audio'){
									$item='<div class="tf_lazy tf_audio_lazy tf_w tf_box">'.$item.'</div>';
								}
								$part=$item;
