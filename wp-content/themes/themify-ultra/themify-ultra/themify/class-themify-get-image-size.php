...
final class Themify_Get_Image_Size{
	...
	private static $MAX_FILE=50;//maximum file convert in 1 php script execution
	...
	public static function getSize($url,$isLocal=false){
		...
		if($isLocal===false){
			self::$method=apply_filters('tf_disable_remote_szie',self::$method);
			static $index=0;
			++$index;
			if(self::$method===null){
		...
		self::$size=self::$imageType=self::$w=self::$TMP_DIR=self::$h=self::$URL=null;
		self::$currentSize=0;
		if(self::$handle!==null){
			fclose(self::$handle);
			self::$handle=null;
	...
	private static function loadFile($url,$isLocal,$size=false){
		if($isLocal!=='curl' && ($isLocal===true || self::$method==='file')){
			...
			self::$currentSize=0;
			self::$w=self::$h=null;
			self::$TMP_DIR=tempnam(get_temp_dir(),'tf_');
		
			if(self::$TMP_DIR===false || file_put_contents(self::$TMP_DIR,'')===false){
				return false;
			}
			$headers = array(
	...
	private static function reCall($type){
		self::$size=self::$imageType=self::$w=self::$h=self::$TMP_DIR=null;
		self::$currentSize=0;
...
function themify_get_image_size($url,$isLocal=false){
	$k=sprintf('%s',crc32($url));
	$dir=themify_upload_dir();
	$dir=rtrim($dir['basedir'],'/').'/tf_image_size/'.$k.'/';
	
	if (is_dir($dir)) {
		if(($handle = opendir($dir))){
			$found=false;
			while (false !== ($entry = readdir($handle))) {//in 90% percent will be first subfolder,with name of size
				if ($entry !=='.' && $entry !== '..' && strpos($entry,$k)===0) {
					$found=str_replace($k.'-','',$entry);
					break;
				}
			}
			closedir($handle);
			if($found!==false){
				$found=explode('X',$found);
				if(isset($found[1])){
					return array('w'=>$found[0],'h'=>$found[1]);
				}
			}
		}
		Themify_Filesystem::delete($dir);//something goes wrong,regenerate again
	}
	$size=Themify_Get_Image_Size::getSize($url,$isLocal);
	if($size!==false){
		$dir.=$k.'-'.$size['w'].'X'.$size['h'];
		mkdir($dir,0755,true);//creating empty direcory to store sizes as direcory names
		return $size;
	}
	return false;
}
