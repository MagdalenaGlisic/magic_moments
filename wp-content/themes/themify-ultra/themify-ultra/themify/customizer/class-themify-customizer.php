...
if (!class_exists('Themify_Customizer')) :
	...
	class Themify_Customizer {
		...
		function customize_control_scripts() {
			...
			// Minicolors JS
			themify_enque_script('themify-colorpicker-js', THEMIFY_METABOX_URI . 'js/jquery.minicolors.min.js', array('jquery'), THEMIFY_VERSION,true);
		...
		function write_stylesheet($delete_empty = true) {
			...
			if (!empty($css_to_save)) {
				...
				update_option('themify_custom_cf_fonts', !empty($this->customizer_cf_fonts) ? $this->customizer_cf_fonts : array() );
			} elseif ($delete_empty) {
				Themify_Filesystem::delete($css_file,'f');
			}
		...
		function enqueue_stylesheet() {
			...
			if($this->test_stylesheet()){
				add_action('tf_load_styles',array($this,'load_css'),10,20);
