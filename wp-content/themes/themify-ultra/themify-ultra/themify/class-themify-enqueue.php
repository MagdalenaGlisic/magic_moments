...
class Themify_Enqueue_Assets {
    ...
    private static $isBuilderActive = false;
    ...
    public static $THEMIFY_CSS_MODULES_URI=THEMIFY_URI . '/css/modules/';
    ...
    public static function init() {
		...
		add_action('themify_builder_load_module_partial',array(__CLASS__,'set_builder_active'),100,1);
    ...
    public static function after_enqueue() {
		...
		if (themify_is_woocommerce_active()) {
			...
			if($is_theme===true && (themify_is_shop() || is_product() || (!is_checkout() && !is_cart() && !is_account_page() && !is_checkout_pay_page() && !is_edit_account_page() && !is_order_received_page() && !is_add_payment_method_page()))){
				...
				global $wp_scripts;
                $js=array('js-cookie','jquery-blockui','wc-add-to-cart','wc-add-to-cart-variation','wc-cart-fragments','woocommerce','wc_additional_variation_images_script','wc-single-product');
				$arr=array();
				foreach ($js as $v) {
					if (isset($wp_scripts->registered[$v]) && wp_script_is($v)) {
						...
						if('jquery-blockui'!==$v){ // Skip deqeue jquery-blockui
                            $wp_scripts->done[]=$v;
                        }
    ...
    public static function add_css($handle, $src, $deps, $ver, $media=false, $in_footer = false) {
		if (!isset(self::$css[$handle],self::$done[$src])) {
			...
			if((self::$isBuilderActive===true && Themify_Builder::$frontedit_active === true) || is_admin()){
				self::$css[$handle] = array('s' => $src, 'v' => $ver);
				if($media!=='all'){
				    self::$css[$handle]['m']=$media;
				}
				if(self::$isBuilderActive===false && is_admin()){
	...
	if (self::$isBuilderActive === false && themify_is_lazyloading() === true) {
	    ?>
	...
	<?php if (self::$themeVersion!==null): ?>
	    ...
	    if ($custom_css = themify_get('setting-custom_css',false,true)):// Custom CSS
		?>
		<!-- custom css -->
    ...
    public static function js_localize(){
	    ...
	    if(self::$themeVersion!==null && !is_admin()){
			...
			if(self::$isBuilderActive===true && is_file(self::$THEME_CSS_MODULES_DIR.'builderActive.css') ){
	...
	if (self::$concateFile !== null) {
			...
			if (!is_file(self::$concateFile)) {
				...
				foreach (self::$css as $k => $v) {
					if ($v !== true && $v!=='footer') {
						...
						$output.='<link rel="preload" href="' . $v['s'] . '?ver=' . $v['v'] . '" as="style"/><link type="text/css" id="' . $k . '-css" rel="stylesheet" href="' . $v['s'] . '?ver=' . $v['v'] . '"/>';
					}
				}
				if($key !== ''){
					clearstatcache();
					if(!is_file(self::$concateFile) && !file_put_contents(self::$concateFile,$str, LOCK_EX)){//maybe another proccess has already created it?
					$key='';
					}
				}
				$str=null;
		...
		$path = '<link rel="preload" href="' . $href. '" as="style"/><link type="text/css" id="themify_concate-css" rel="stylesheet" href="' . $href. '"/>';
	    } else {
		$path = $output;
	    }
	    $output='';
		    ...
		    if($key!==''){
			    ...
			    if (!is_file($mobileDir)) {
				    ...
				    foreach (self::$mobileMenuCss as $k => $v) {
					    ...
					    $output.='<link rel="preload" href="' . $v . '?ver=' . self::$themeVersion . '" media="' . $media . '" as="style"/><link type="text/css" id="tf-mobile-' . $k . '-css" rel="stylesheet" href="' . $v . '?ver=' . self::$themeVersion . '" media="' . $media . '"/>';
				    }
				    self::$mobileMenuCss=null;
				    if($mobileDir!==null){
					clearstatcache();
					if (!is_file(self::$concateFile) && !file_put_contents($mobileDir, $str, LOCK_EX)) {//maybe another proccess has already created it?
					    $mobileDir=null;
					}
				    }
				    $str=null;
				...
				$path.= '<link rel="preload" href="' . $mobileDir. '" as="style" media="' . $media . '"/><link type="text/css" id="themify_mobile_concate-css" rel="stylesheet" href="' . $mobileDir. '" media="' . $media . '"/>';
			    }
			    $output=$mobileDir=$key=null;
		    }
	    }
	    foreach ($medias as $k => $v) {
			$path.='<link rel="preload" href="' . $v['s'] . '?ver=' . $v['v'] . '" media="' . $v['m']. '" as="style"/><link type="text/css" id="' . $k . '-css" rel="stylesheet" href="' . $v['s'] . '?ver=' . $v['v'] . '" media="' .$v['m'] . '"/>';
	    }
	    $medias = null;
		...
		if ( ! wp_script_is( 'themify-main-script' )) {
				...
				if(!is_admin()){
					...
					if(self::$themeVersion!==null){
						...
						self::addPreLoadJs($src,$ver);
						if((is_single() || is_page()) && comments_open() && get_option('thread_comments') == 1){
	...
	public static function lazy_init(){
		self::$isBuilderActive = class_exists('Themify_Builder') ? (Themify_Builder::$frontedit_active === true || Themify_Builder_Model::is_front_builder_activate() === true) : false;
	...
	public static function set_builder_active($args){
		self::$isBuilderActive = true;
	}
	...
	*/
	...
	public static function loadGuttenbergCss($content){
		if(function_exists('has_blocks') && !empty(self::$guttenbergCss) ){
			if($content!=='' && has_blocks(get_the_id()) && strpos($content,'wp-block',3)!==false){
				global $wp_styles,$wp_version;
