...
function portfolio_comments_open( $open, $post_id ) {
	...
	$open = !empty($post) && 'portfolio' === $post->post_type && themify_check( 'setting-portfolio_comments',true )?true:$open;
	return $open;
}
					...
					if(themify_check( $k )){
						...
						elseif ( $prop === 'background-image' && 'default' !== $v ) {
							...
							if(isset($v2)){
								...
								if($v2==='fullcover' &&  $selector==='.skin-styles:before'){
									$css[$selector]['background-size'].='content:"";position:fixed;width:100%;height:100%;left:0;top:0;pointer-events:none;z-index:-1';
								}
								else{
									$css[$selector]['background-size'].= 'background-attachment:fixed';
								}
								$css[$selector][$prop] = $prop .': url(' . $v . ')';
							}
						}
...
function themify_theme_add_section_loader() {
	...
	if ( !Themify_Builder_Model::is_front_builder_activate() && (page_loader_status() || themify_theme_is_fullpage_scroll()) ) {
		...
		$media=(!page_loader_status() && themify_theme_is_fullpage_scroll()) && 'on'!==themify_get( 'section_scrolling_mobile' )?'screen and (min-width:'.themify_get_breakpoints('tablet')[1].'px)':null;
